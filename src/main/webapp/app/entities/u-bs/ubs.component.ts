...
currentAccount: any;
    ...
    transition() {
        this.router.navigate(['/ubs'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: UBS) {
        return item.id;
    }
    registerChangeInUBS() {
        this.eventSubscriber = this.eventManager.subscribe('uBSListModification', (response) => this.loadAll());
    }
